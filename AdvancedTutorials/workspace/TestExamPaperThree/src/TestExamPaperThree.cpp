...
//the before a memeber means the name will not be changes or mainpulated, not even within the constructor.
//the const in the parameter followed by the reference means the string will be passed by reference and will not be change or manipulated. means the string that is passed in is unchanagable
//the const in right side of the function means the function cannot be changed or manipulated, any object and assigns into these will not be able to manipulate the function and its members.
//
...
//the virtual keyword means the memebre functions with virtual can re-implemented or re-definned, duplicated or overriden by any derived classes of Employees
...
public:
	...
	HourlyEmployee(const string &n, double hours): Employee(n), hours(hours) {}
	// add pay for working h hours
	virtual void work_hours(double h) {
		//hours = h;
		pay += hours*h;
...
public:
	CommissionEmployee(const string &n, double amount): Employee(n), amount(amount/100) {}
...
class Payroll : public HourlyEmployee, public CommissionEmployee {

public :
	Payroll(const string &n, double hours, double amount):
		Employee(n),
		HourlyEmployee(n, hours),
		CommissionEmployee(n, amount) {
	}
};
